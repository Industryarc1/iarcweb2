...
class SearchController extends IarcfbaseController
{
	...
	public function actionSearch()
	{
		...
		$arrGet['searchKey'] = filter_var($arrGet1['searchKey']);
		if (isset($arrGet1['tag'])) {
			$arrGet['tag'] = filter_var($arrGet1['tag']);
		} else {
			$arrGet['tag']	= "All";
		}
	...
	public function actionSearchReport($reportTitle)
	{
		...
		$sql = "SELECT `title`, `short_descr`, `curl`, `meta_title`, `dup_inc_id` FROM `zsp_posts` WHERE status=1 AND `title` LIKE " . $keyTitle1 . " LIMIT 5"; 
				/* UNION 
				(SELECT `title`, `short_descr`, `curl`, `meta_title`, `dup_inc_id` FROM `zsp_posts` WHERE status=1 AND MATCH(`title`) AGAINST (" . $keyTitle2 . " IN NATURAL LANGUAGE MODE)) "; */
	...
	*/
	...
	public function actionSearchPressRelease($title)
	{
		...
		$sql = "SELECT `title`, `short_descr`, `seo_keyword`, `meta_title`, `prod_id` FROM `zsp_prs` WHERE  (`title` LIKE '%$title%' OR `title` LIKE '%$newtitle%' OR `title` LIKE " . $keyTitle1 . " OR `title` LIKE ". $keyTitle2 .") LIMIT 5";
				/* UNION 
				(SELECT `title`, `short_descr`, `seo_keyword`, `meta_title`, `prod_id` FROM `zsp_prs` WHERE MATCH(`title`) AGAINST (" . $keyTitle2 . " IN NATURAL LANGUAGE MODE)) LIMIT 20"; */
	...
	public function actionSearchWhitepapers($title)
	{
		...
		$sql = "SELECT `title`, `short_descr`, `seo_keyword`, `meta_title`, `prod_id` FROM `zsp_whitepapers` WHERE (`title` LIKE " . $keyTitle1 . ") LIMIT 20"; 
				/* UNION 
