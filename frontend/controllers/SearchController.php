...
*/
	...
	public function actionSearchReport($reportTitle){
		...
		$sql = "(SELECT `title`, `short_descr`, `curl`, `meta_title`, `dup_inc_id` FROM `zsp_posts` WHERE status=1 AND (`title` LIKE ".$keyTitle1." OR (REGEXP_REPLACE(FROM_BASE64(`description`), '<img[^>]*>', '') LIKE '%$reportTitle%'))) LIMIT 10";
