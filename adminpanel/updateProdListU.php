...
if(isset($_POST["addArticle"])){
	...
	if(@$_POST["hid_article_id"]!=""){
		$query = "update zsp_posts set cat=?,subcat=?,title=?,code=?,short_descr=?,description=?,table_of_content=?,report_type=?,pub_date=?,report_del=?,del_time=?,file_format=?,no_pages=?,slp=?,clp=?,image=?,status=?,curl=?,meta_title=?,meta_keywords=?,meta_descr=?,seo_keyword=?,related=?,dt_created=now(),taf=?,cc=?,pages=?,cat_but=?,atag=?,dup_inc_id=?,cbreadcrumb=? where inc_id=?";
		...
		$query = "insert into zsp_posts(cat,subcat,title,code,short_descr,report_type,pub_date,report_del,del_time,file_format,no_pages,slp,clp,image,status,curl,meta_title,meta_keywords,meta_descr,seo_keyword,related,dt_created,cc,pages,atag,dup_inc_id,cbreadcrumb,publish_type,is_publish)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,now(),?,?,?,?,?,'1','1')";
		if ($stmt = $mysqli->prepare($query)){
			$sssssssssssssssssssssss='ssssssssssssssssssssssssss';
			$stmt->bind_param($sssssssssssssssssssssss, $cat,$subcat,$title,$rcode,$short_descr,$report_type,$pub_date,$report_del,$del_time,$file_format,$no_pages,$slp,$clp,$image,$status,$curl,$meta_title,$meta_keywords,$meta_descr,$seo_keyword,$rr,$cc,$pages,$atag,$rid,$cbreadcrumb);
